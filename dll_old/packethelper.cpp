...
{
	if (lpMsg == NULL || lpMsg[0] == 0)
		return FALSE;

	const int LEN = strlen(lpMsg);
	BYTE* pBuffer = new BYTE[LEN + 6];

	pBuffer[0] = 0x15;
	memcpy(pBuffer + 1, &wPlayerID, 2);
	memcpy(pBuffer + 3, lpMsg, LEN);
	memset(pBuffer + LEN + 3, 0, 3);

	server->GameSendPacketToServer(pBuffer, LEN + 6);
