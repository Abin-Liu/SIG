...
// by Abin
#define PARAM_D2WND			"-d2wnd"
#define PARAM_D2EXE			"-d2exe"
...
BOOL ParseCmdLine(LPSTR lpszCmdLine, HWND& hD2Wnd, LPSTR lpszD2Exe);
...
// by Abin: in case we have parameter for d2 window title
HWND hD2Wnd = NULL;
char g_szD2Exe[MAX_PATH + 1] = "";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	// by Abin: allowing user to specify the D2 window name from command line, 
	// this is a must for multiple instances running in Pindlebot 3.00 or later.
	ParseCmdLine(lpCmdLine, hD2Wnd, g_szD2Exe);
	...
	for(x=0; x<NumWindows; x++) {
		...
		if (g_szD2Exe[0])
			PatchAddress = GetBaseAddress(hd.pid, g_szD2Exe); // try user specified exe first
		else
		...
		{ 
			...
			{ 
				...
				if (!PatchAddress)
				{
					MessageBox(NULL, "Unable to find a patch location.", "Loader Error!", MB_ICONERROR); return -1;
				}
			}
		}
...
BOOL ParseCmdLine(LPSTR lpszCmdLine, HWND& hD2Wnd, LPSTR lpszD2Exe)
{
	...
	for (int i = 0; lpszCmdLine[i]; i++)
		tolower(lpszCmdLine[i]);

	// get d2 hwnd
	LPCSTR p = strstr(lpszCmdLine, PARAM_D2WND);
	if (p)
	{
		p += strlen(PARAM_D2WND);
		if (*p)
		{
			p++;
			char szD2Wnd[9] = "";
			strncpy(szD2Wnd, p, 8);
			hD2Wnd = (HWND)strtoul(szD2Wnd, NULL, 16);
			if (!::IsWindow(hD2Wnd))
				hD2Wnd = NULL;
		}		
	}
	...
	// get d2 exe name
	p = strstr(lpszCmdLine, PARAM_D2EXE);
	if (p)
	{
		p += strlen(PARAM_D2EXE);
		if (*p)
			p++;
