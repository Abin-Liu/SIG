							 ...
							 LPCTSTR lpDictionary) const
{
	...
	ini.WriteBool(_T("General"), _T("Use Locale"), m_bUseLocale);
	ini.WriteInt(_T("General"), _T("Locale"), m_nLocaleType);
	ini.WriteBool(_T("General"), _T("Use MPQ"), m_bUseMpq);
	ini.WriteString(_T("General"), _T("MPQ File"), m_sMpqFile);
	ini.WriteBool(_T("General"), _T("LQ"), m_bLQ);
	ini.WriteBool(_T("General"), _T("NS"), m_bNS);
	ini.WriteBool(_T("General"), _T("Res800"), m_bRes800);
	...
	ini.WriteBool(_T("General"), _T("noplugin"), m_bNoPlugin);
	ini.WriteBool(_T("General"), _T("use pdir"), m_bPdir);
	ini.WriteString(_T("General"), _T("pdir"), m_sPdirPath);
							 ...
							 LPCTSTR lpDictionary)
{
	...
	m_bUseLocale = ini.GetBool(_T("General"), _T("Use Locale"), FALSE);
	m_nLocaleType = ini.GetInt(_T("General"), _T("Locale"), 0);
	m_bUseMpq = ini.GetBool(_T("General"), _T("Use MPQ"), FALSE);
	m_sMpqFile = ini.GetString(_T("General"), _T("MPQ File"), _T(""));
	m_bLQ = ini.GetBool(_T("General"), _T("LQ"), FALSE);
	m_bNS  = ini.GetBool(_T("General"), _T("NS"), TRUE);
	m_bRes800  = ini.GetBool(_T("General"), _T("Res800"), TRUE);
	...
	m_bNoPlugin = ini.GetBool(_T("General"), _T("noplugin"), TRUE);
	m_bPdir = ini.GetBool(_T("General"), _T("use pdir"), TRUE);
	m_sPdirPath = ini.GetString(_T("General"), _T("pdir"), _T("plugin"));
...
BOOL CD2ConfigData::LaunchDiablo2()
{
	...
	CString str = GetParamLine();
	char szParam[1024] = "";
	_tcsncpy(szParam, str, 1023);
    ::CreateProcess(m_sD2Executable, 
     szParam,
     NULL,NULL,FALSE, 
     0, 
     NULL, 
...
{	
	...
	return LoadD2Hackit(hwnd, ff.GetFileName(), GetHackDir() + _T("\\D2Hackit.dll"));
/*
	CString sParams;
...
CString CD2ConfigData::GetParamLine() const
{
	CString sLine = _T("-w -nonotify -skiptobnet ");
	...
	{
		if (m_bPdir && !m_sPdirPath.IsEmpty())
		{
			sLine += _T("-pdir \"");
			sLine += m_sPdirPath;
			sLine += _T("\" ");
	...
	if (m_bUseLocale)
	{
		if (m_nLocaleType == 0)
			sLine += _T("-locale \"ENG\" ");
		else if (m_nLocaleType == 1)
			sLine += _T("-locale \"CHI\" ");
	}

	if (m_bUseMpq && !m_sMpqFile.IsEmpty())
	{
		sLine += _T("-mpq ");
		sLine += m_sMpqFile + _T(" "); 
	}
...
BOOL CD2ConfigData::LeaveGame() const
{
	...
	VirtualProtect(LPVOID(0x6FAA0000+0x000D4538), 4, oldprot, NULL);
	::CloseHandle(hProc);
...
CString CD2ConfigData::GetD2FullCmdLine() const
{
	return m_sD2Executable + _T(' ') + GetParamLine();
}

BOOL CD2ConfigData::FindD2HackitInPdir(CString& sFile) const
{
	if (m_bNoPlugin)
		return FALSE;

	CString spdir = _T("plugin");
	if (m_bPdir && !m_sPdirPath.IsEmpty())
		spdir = m_sPdirPath;

	if (spdir.Find(':') == -1) // abs/relative path?
		spdir = GetD2Path() + _T('\\') + spdir;

	CFileFind ff;
	BOOL bFound = ff.FindFile(spdir + _T("\\D2Hackit.dll"));
	if (bFound)
	{
		ff.FindNextFile();
		sFile = ff.GetFilePath();
	}
	ff.Close();
	return bFound;
}
