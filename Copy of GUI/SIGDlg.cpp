...
void CSIGDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LOCATION, m_wndLocation);
	DDX_Control(pDX, IDC_CLASSNAME, m_wndCharName);
	DDX_Control(pDX, IDC_PACKETSSND, m_wndPacketsSnd);
	DDX_Control(pDX, IDC_PACKETSRCV, m_wndPacketsRcv);
	DDX_Control(pDX, IDC_HEARTBEAT, m_wndHeartBeat);
	DDX_Control(pDX, IDC_ANTIIDLE, m_wndAntiIdle);
...
BEGIN_MESSAGE_MAP(CSIGDlg, CDialog)
	...
	ON_MESSAGE(WM_SIG_GAMEJOIN, OnSIGJoin)
	ON_MESSAGE(WM_SIG_GAMELEAVE, OnSIGLeave)
	...
	ON_MESSAGE(WM_SIG_LOCATION, OnSIGLocation)
...
BOOL CSIGDlg::OnInitDialog()
{
	...
	m_wndLocation.DrawTransparent();
	m_wndLocation.SetIcon(IDI_LIGHT_LC_GRAY);

	m_wndPacketsRcv.DrawTransparent();
	m_wndPacketsRcv.SetIcon(IDI_LIGHT_RV_GRAY);

	m_wndPacketsSnd.DrawTransparent();
	m_wndPacketsSnd.SetIcon(IDI_LIGHT_ST_GRAY);

	m_wndHeartBeat.DrawTransparent();
	m_wndHeartBeat.SetIcon(IDI_LIGHT_HB_GRAY);

	m_wndAntiIdle.DrawTransparent();
	m_wndAntiIdle.SetIcon(IDI_LIGHT_AI_GRAY);
...
LRESULT CSIGDlg::OnSIGHeartBeat(WPARAM wParam, LPARAM lParam)
{
	if (!m_bUseD2JSP && m_bStarted)
	{
		m_wndHeartBeat.SetIcon(IDI_LIGHT_HB_RED);
		SetTimer(ID_TMR_HEARTBEAT, 500, NULL);
	}
	return (LRESULT)0;
}
...
LRESULT CSIGDlg::OnSIGPktRcv(WPARAM wParam, LPARAM lParam)
{
	if (!m_bUseD2JSP && m_bStarted)
	{
		m_wndPacketsRcv.SetIcon(IDI_LIGHT_RV_GREEN);
		SetTimer(ID_TMR_PACKETSRCV, 250, NULL);
	}
	return (LRESULT)0;
}
...
LRESULT CSIGDlg::OnSIGPktSnd(WPARAM wParam, LPARAM lParam)
{
	if (!m_bUseD2JSP && m_bStarted)
	{
		m_wndPacketsSnd.SetIcon(IDI_LIGHT_ST_GREEN);
		SetTimer(ID_TMR_PACKETSSND, 250, NULL);
	}
	return (LRESULT)0;
}
...
LRESULT CSIGDlg::OnSIGAntiIdle(WPARAM wParam, LPARAM lParam)
{
	if (!m_bUseD2JSP && m_bStarted)
	{
		m_wndAntiIdle.SetIcon(IDI_LIGHT_AI_RED);
		SetTimer(ID_TMR_ANTIIDLE, 250, NULL);
	}
	return (LRESULT)0;
}
...
void CSIGDlg::OnTimer(UINT nIDEvent) 
{
	...
	case ID_TMR_INGAMETIME:
		...
		if (m_bUseD2JSP && m_cfg.m_bInGameLimit && m_cfg.m_dwInGameLimitLen < m_stats.GetCurrentGameTime())
		{
			if (!m_bPaused)
			{
	...
	case ID_TMR_HEARTBEAT:
		KillTimer(ID_TMR_HEARTBEAT);
		m_wndHeartBeat.SetIcon(IDI_LIGHT_HB_GRAY);
		break;

	case ID_TMR_PACKETSRCV:
		KillTimer(ID_TMR_PACKETSRCV);
		m_wndPacketsRcv.SetIcon(IDI_LIGHT_RV_GRAY);
		break;

	case ID_TMR_PACKETSSND:
		KillTimer(ID_TMR_PACKETSSND);
		m_wndPacketsSnd.SetIcon(IDI_LIGHT_ST_GRAY);
		break;

	case ID_TMR_ANTIIDLE:
		KillTimer(ID_TMR_ANTIIDLE);
		m_wndAntiIdle.SetIcon(IDI_LIGHT_AI_GRAY);
		break;

	case ID_TMR_PLAYERMOVING:
		PlayerMoving();
		break;
...
void CSIGDlg::TmrProc_Main()
{
	...
	case ST_LAUNCH_DIABLO:
		...
		if (m_cfg.FindD2HackitInPdir(sMsg))
		{
			Terminate();
			m_wndLabel.SetTextColor(RGB(255, 255, 0));
			ShowInfo(_T("-pdir目录中发现D2Hackit.dll，SIG中止"));
			CString sErr;
			sErr.Format(_T("在用户指定的-pdir目录中发现D2Hackit.dll，SIG无法继续运行。\n\n%s。"), sMsg);
			MessageBox(sErr, _T("SIG错误"), MB_ICONEXCLAMATION | MB_OK);
			break;
		}
...
BOOL CSIGDlg::PreTranslateMessage(MSG* pMsg)
{
		...
		|| pMsg->message == WM_MBUTTONUP)*/
	{
		...
		m_wndLocation.GetWindowRect(&rcIgnore);
		if (rcIgnore.PtInRect(pMsg->pt))
			return TRUE;

		m_wndPacketsRcv.GetWindowRect(&rcIgnore);
		if (rcIgnore.PtInRect(pMsg->pt))
			return TRUE;

		m_wndPacketsSnd.GetWindowRect(&rcIgnore);
		if (rcIgnore.PtInRect(pMsg->pt))
			return TRUE;

		m_wndHeartBeat.GetWindowRect(&rcIgnore);
		if (rcIgnore.PtInRect(pMsg->pt))
			return TRUE;

		m_wndAntiIdle.GetWindowRect(&rcIgnore);
		if (rcIgnore.PtInRect(pMsg->pt))
			return TRUE;
	}
...
void CSIGDlg::OnGameLeave(BOOL bRecord)
{
	...
	m_bGameJoined = FALSE;
	m_mem.SetLong(0, MEM_SIG_MAXSTAY); // disable auto-quit

	KillTimer(ID_TMR_HEARTBEAT);
	KillTimer(ID_TMR_PACKETSRCV);
	KillTimer(ID_TMR_PACKETSSND);
	KillTimer(ID_TMR_ANTIIDLE);
	KillTimer(ID_TMR_PLAYERMOVING);
...
void CSIGDlg::OnGameJoin(BYTE iCharClass)
{
	...
	if (m_bStarted)
	{
		...
		m_mem.SetLong(dwStay, MEM_SIG_MAXSTAY);
		m_dwPrevLocation = 0xffffffff;
		m_bGameJoined = TRUE;
		m_bMovingOn = TRUE;
		MoveOn(ST_IN_GAME);
		m_wndLocation.SetIcon(IDI_LIGHT_LC_GREEN);
		...
		{
			// d2hackit
			TCHAR szCharName[65] = _T("");
			m_mem.GetString(szCharName, 64, MEM_SIG_CHARNAME);
			sPlayerName = szCharName;
			sClass = GetCharClassName(iCharClass, TRUE);
			str.Format(_T("%s %s"), GetCharClassName(iCharClass, FALSE), szCharName);
		}
		m_wndCharName.SetText(str);

		CString sMsg;

		if (m_mem.GetLong(MEM_SIG_MAXSTAY) == 0)
			sMsg = _T("进入游戏成功");
		else
			sMsg.Format(_T("进入游戏成功(停留时限%d秒)"), m_mem.GetLong(MEM_SIG_MAXSTAY));
		ShowInfo(sMsg);
...
{
	// This function is only for D2JSP
	if (!m_bUseD2JSP)
		return;
...
void CSIGDlg::TmrProc_Player()
{
	...
	DWORD dwLocation = m_spy.GetPlayer()->dwPlayerLocation;
	if (m_bGameJoined && dwLocation != m_dwPrevLocation)
	{
		m_dwPrevLocation = dwLocation;
		if (dwLocation == 0x05)
		{
			KillTimer(ID_TMR_PLAYERMOVING);
			m_wndLocation.SetIcon(IDI_LIGHT_LC_GREEN);
		...
		else if (dwLocation == 0x01)
		{
			KillTimer(ID_TMR_PLAYERMOVING);
			m_wndLocation.SetIcon(IDI_LIGHT_LC_RED);
		}
		else
		{
			SetTimer(ID_TMR_PLAYERMOVING, 100, NULL);
		}
	}
}
...
void CSIGDlg::HideLightsForD2JSP()
{
	...
	m_wndHeartBeat.ShowWindow(!m_bUseD2JSP);
	m_wndPacketsRcv.ShowWindow(!m_bUseD2JSP);
	m_wndPacketsSnd.ShowWindow(!m_bUseD2JSP);
	m_wndAntiIdle.ShowWindow(!m_bUseD2JSP);
}

LRESULT CSIGDlg::OnSIGJoin(WPARAM wParam, LPARAM lParam)
{
	OnGameJoin((BYTE)wParam);
	return (LRESULT)0;
}

LRESULT CSIGDlg::OnSIGLeave(WPARAM wParam, LPARAM lParam)
{	
	OnGameLeave();
	return (LRESULT)0;
}

LRESULT CSIGDlg::OnSIGLocation(WPARAM wParam, LPARAM lParam)
{
	DWORD dwLocation = (DWORD)wParam;
	if (m_bGameJoined && dwLocation != m_dwPrevLocation)
	{
		m_dwPrevLocation = dwLocation;
		if (dwLocation == 0x05)
